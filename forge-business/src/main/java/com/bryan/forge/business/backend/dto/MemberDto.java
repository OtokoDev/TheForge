...
@Serdeable
public record MemberDto(UUID userId, String username, MembershipRole role, int version) {
    public static MemberDto from(Membership m) {
        return new MemberDto(m.getUser().getId(), m.getUser().getUsername(), m.getRole(), m.getVersion());
    }
}
